                ...
                <div class="nav-body">
                    <div class="nav-body-wrapper">
                        <div class="container-fluid">
                            <form  action="{{url('/updateplayers').'/'.$players->id}}" method="post"  enctype="multipart/form-data"/>
                            @csrf
                            <div class="row">
                                <div class="col-lg-8">
...
{{--                                                            @if(Auth::user()->role==1)--}}
                                                            @foreach ($categories as $category)
                                                                <option value="{{$category->id }}" @if($players->category_id==$category->id) {{'selected'}} @endif {{ (collect(old('category_id'))->contains($category->id)) ? 'selected':'' }}>{{$category->name }} </option>
                                                            @endforeach
                                            ...
                                            @foreach ($paid as $pd)
                                                <div class="form-group">
                                                    <label class="custom-control custom-radio ks-radio">
                                                        <input type="radio" class="custom-control-input" name="paid_id" value="{{$pd->id }}" @if($players->paid_id==1&&$pd->id==1) checked @endif @if($players->paid_id==2&&$pd->id==2) checked @endif @if($players->paid_id==3&&$pd->id==3) checked @endif >
                                                        <span class="custom-control-indicator"></span>
                                                        <span class="custom-control-description">{{$pd->name }}</span>
                                                    </label>
                                                    @if($pd->id==1)
                                                        <div id="complete" @if($players->paid_id==2||$players->paid_id==3) style="display: none" @else style="display: inline-block" @endif>
                                                            <label>قيمة الدفع:</label>
                                                            <input type="number" class="form-control paid_value" name="paid_value1" @if($players->paid_id==1) value="{{$players->paid_value }}" @endif>
                                                            <label class="sh_margin">شيكل</label>
                                                        </div>
                                                    @endif

                                                    @if($pd->id==2)
                                                        <div id="part" @if($players->paid_id==1||$players->paid_id==3) style="display: none" @else style="display: inline-block" @endif>
                                                            <label>قيمة الدفع:</label>
                                                            <input type="number" class="form-control paid_value" name="paid_value2" @if($players->paid_id==2) value="{{$players->paid_value }}" @endif>
                                                            <label class="sh_margin">شيكل</label>

                                                            <label>باقي حساب:</label>
                                                            <input type="number" class="form-control paid_value" name="paid_remainder" @if($players->paid_id==2) value="{{$players->paid_remainder }}"  @endif>
                                                            <label class="sh_margin">شيكل</label>
                                                        </div>
                                                    @endif
                                            ...
                                            <div class="row">
                                                <div class="form-group col-lg-6">
                                                    ...
                                                    <input class="form-control flatpickr" name="reg_date" value="{{$players->reg_date }}">
                                                </div>
                                                <div class="form-group col-lg-6">
                                                    <label>تاريخ انتهاء التسجيل</label>
                                                    <input class="form-control flatpickr" name="reg_end_date" value="{{$players->reg_end_date }}">
                                                </div>
                                            </div>
                                            <div class="form-group">
                                                <label>ملاحظات</label>
                                                <textarea rows="3" id="maxlength-area" name="notes" class="form-control" >{{$players->notes }}</textarea>
                                            </div>
                                            <script>
                                            ...
                                            <div class="form-group">
                                                <label class="custom-control custom-checkbox">
                                                    <input type="checkbox" class="custom-control-input" value="1" name="activated_id" @if($players->activated_id==1) checked @endif>
                                                    <span class="custom-control-indicator"></span>
                                                ...
                                                @foreach ($level as $lv)
                                                    <label class="custom-control custom-radio ks-radio">
                                                        <input type="radio" class="custom-control-input" name="level_id" value="{{$lv->id }}" @if($players->level_id==1&&$lv->id==1) checked @endif @if($players->level_id==2&&$lv->id==2) checked @endif @if($players->level_id==3&&$lv->id==3) checked @endif >
                                                        <span class="custom-control-indicator"></span>
                                        ...
                                        <div class="card-block">
                                            <div class="form-group">
                                                ...
                                                <div style=" overflow: hidden;">
                                                    <img src="{{asset($players->img )}}"  id="image-select" alt=" ">
                                                </div>
                                            </div>
                                            <label class="custom-control custom-checkbox">
